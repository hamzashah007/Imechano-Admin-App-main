...
import 'package:imechano_admin/ui/bottombar/location_map.dart';
...
class AddGarageProfile extends StatefulWidget {
  const AddGarageProfile({Key? key}) : super(key: key);
...
class _AddGarageProfileState extends State<AddGarageProfile> {
  ...
  TextEditingController garage_profile = TextEditingController();
  TextEditingController email = TextEditingController();
  TextEditingController mobileno = TextEditingController();
  TextEditingController crno = TextEditingController();
  ...
  void CreateGrageProfile(File Image) async {
    ...
    try {
      final uri = Uri.parse(Config.apiurl + "add-garage-profile");

      var request = http.MultipartRequest("POST", uri);
      request.fields['name'] = garageName.text;
      request.fields['garage_profile'] = garage_profile.text;
      request.fields['email'] = email.text;
      request.fields['mobile_no'] = mobileno.text;
      request.fields['address'] = address.text;
      request.fields['city'] = city.text;
      request.fields['crno'] = crno.text;
      request.fields['longitude'] = longitude.text;
      request.fields['latitude'] = latitude.text;
      request.fields['mobileno'] = "+97" + mobileno.text;
      ...
      if (response.statusCode == 200) {
        print('Garage data sent successfully!');
        Loader().hideLoader(context);

        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (BuildContext context) => GarageProfile(),
          ),
        );
      } else {
        print(
  ...
  getAddress() async {
    ...
    setState(() {
      address.text = prefs.getString('address') ?? "Null";
      longitude.text = prefs.getString("lontitude") ?? "Null";
      latitude.text = prefs.getString("latitude") ?? "Null";
      city.text = prefs.getString("city") ?? "Null";
    });
  }
                    ...
                    Expanded(
                      ...
                      child:
                        ...
                        child: SingleChildScrollView(
                          child: Padding(
                            padding: EdgeInsets.fromLTRB(25, 0, 22, 0),
                            child: Column(
                              children: [
                                SizedBox(
                                  height: 15.h,
                                ...
                                Row(
                                  ...
                                  children: [
                                    textFiled("Garage Name ", 175, garageName),
                                    textFiled(
                                        "Garage Profile", 175, garage_profile),
                                  ],
                                ),
                                Padding(
                                  padding:
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      textFiled("Email", 300, email),
                                      // textFiled("Mobile No", 175, mobileno),
                                    ],
                                  ),
                                ),
                                Row(
                                ...
                                Row(
                                  children: [
                                    Padding(
                                      padding:
                                          const EdgeInsets.fromLTRB(5, 0, 0, 0),
                                      child: textFiled("CR No", 175, crno),
                                    ),
                                    SizedBox(
                                      width: 40.w,
                                    ),
                                ...
                                GestureDetector(
                                  onTap: () {
                                    if (garageName.text.isEmpty ||
                                        garage_profile.text.isEmpty ||
                                        email.text.isEmpty ||
                                        crno.text.isEmpty ||
  ...
  Widget textFiled(String title, int size, TextEditingController controller) {
    return Container(
  ...
  Widget textFieldWithPrefix(
      String prefix, String title, int size, TextEditingController controller) {
    return Container(
          ...
          Container(
            ...
            decoration: BoxDecoration(
              border: Border(
                  right: BorderSide(
                      color: Colors
                          .grey)), // Optional: Add a border to separate the prefix
            ),
            child: Text(
              prefix,
              style: TextStyle(fontSize: 13, fontFamily: 'Poppins3'),
            ),
          ),
          Expanded(
            child: TextField(
